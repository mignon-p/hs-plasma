...
Output files are created if they do not exist, and are overwritten if
they do exist.

Output pools are created if they do not exist.  Created pools are
S<1 MiB> with default options.
...
This can also be used on file inputs, as well.  This is implemented by
skipping slawx in the file until the desired index is reached.  (Slawx
in files are numbered starting with 0.)
...
Print the second slaw from C<stuff.yaml> to stdout in "spew" format.
This is accomplished by using B<-i1> to start with the second slaw in
the file (slawx in a file are numbered starting with zero), and then
B<-c1> is used so that only one slaw is printed.

    slawcat -c1 -i1 stuff.yaml -s -o -
